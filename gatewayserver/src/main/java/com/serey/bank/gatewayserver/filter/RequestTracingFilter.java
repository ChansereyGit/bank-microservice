package com.serey.bank.gatewayserver.filter;
...
public class RequestTracingFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(isCorrelationIdPresent(requestHeader)) {
            log.debug("sereybank-corelation-id found in RequestTracingFilter {}",
                    filterUtility.getCorrelationId(requestHeader));
        }else {
            String correlationId = generateCorrelationId();
            exchange = filterUtility.setCorrelationId(exchange, correlationId);
            log.debug("sereybank-corelation-id generated in RequestTracingFilter {}",
                    correlationId);
        }
